    ...
    <div className="flex min-h-screen items-center justify-center px-6"
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-3 text-lg font-semibold" style={{ color: "var(--text-1)" }}>Create account</h1>
          <p style={{ fontSize: "13px", color: "var(--text-4)", marginTop: "4px" }}>Join Foundry</p>
        </div>
        ...
        <form onSubmit={signup}>
          <div className="overflow-hidden rounded-2xl"
            ...
            ].map(({ label, value, set, type, placeholder, auto }, i, arr) => (
              <div key={label}
                className={i < arr.length - 1 ? "border-b px-4 py-3" : "px-4 py-3"}
                style={{ borderColor: "var(--border-subtle)" }}>
                <label style={{ fontSize: "10px", color: "var(--text-4)", letterSpacing: "0.06em", textTransform: "uppercase" }}>
                  {label}
                </label>
                <input
          ...
          <button type="submit" disabled={loading}
            className="mt-4 w-full rounded-xl py-2.5 text-[14px] font-medium"
            style={{
